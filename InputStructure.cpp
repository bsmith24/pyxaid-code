...
InputStructure::InputStructure( )
{
  ...
  string line, key, value, basedir = "";
  string::size_type pos;

  while( getline( cin, line ) ) {

    pos = line.find( '#' );
    if( pos != line::npos ) {                 // Erase comments marked with '#'
      line.erase( pos, line::npos - pos );
    }

    for( int i = 0, count = 0; line[i]; i++) {   // Remove all spaces
      if( line[i] != ' ' ) line[count++] = line[i];
    }
    line[count] = '\0';

    if( line.length() == 0 ) break;  // Bail for blank lines

    pos = line.find( '=' );          // break key=value
    ...
    else if(key=="Ham_im_suffix"){ Ham_im_suffix = value;  is_Ham_im_suffix = 1; }

    else if(key=="Hprime_x_prefix"){ Hprime_x_prefix = basedir + value; is_Hprime_x_prefix = 1; }
    else if(key=="Hprime_y_prefix"){ Hprime_y_prefix = basedir + value; is_Hprime_y_prefix = 1; }
    else if(key=="Hprime_z_prefix"){ Hprime_z_prefix = basedir + value; is_Hprime_z_prefix = 1; }
    else if(key=="Hprime_x_suffix"){ Hprime_x_suffix = value; is_Hprime_x_suffix = 1; }
    else if(key=="Hprime_y_suffix"){ Hprime_y_suffix = value; is_Hprime_y_suffix = 1; }
    else if(key=="Hprime_z_suffix"){ Hprime_z_suffix = value; is_Hprime_z_suffix = 1; }


//    else if(key=="nac_re_prefix"){ nac_re_prefix = value;  is_nac_re_prefix = 1; }
//    else if(key=="nac_re_suffix"){ nac_re_suffix = value;  is_nac_re_suffix = 1; }
//    else if(key=="nac_im_prefix"){ nac_im_prefix = value;  is_nac_im_prefix = 1; }
//    else if(key=="nac_im_suffix"){ nac_im_suffix = value;  is_nac_im_suffix = 1; }
//    else if(key=="overlap_re_prefix"){ overlap_re_prefix = value;  is_overlap_re_prefix = 1; }
//    else if(key=="overlap_re_suffix"){ overlap_re_suffix = value;  is_overlap_re_suffix = 1; }
    else if(key=="energy_in_one_file"){ energy_in_one_file = value; is_energy_in_one_file = 1; }
    else if(key=="scratch_dir"){ scratch_dir = value; is_scratch_dir = 1; }

    else if(key=="read_couplings") { read_couplings = value; is_read_couplings = 1; }
    else if(key=="read_overlaps") { read_overlaps = value; is_read_overlaps = 1; }
//    else if(key=="many_electron_algorithm"){ many_electron_algorithm = value; 1; }
    else if(key=="namdtime"){ namdtime = value; is_namdtime = 1; }
    else if(key=="sh_algo"){ sh_algo = value; is_sh_algo = 1; }
    else if(key=="num_sh_traj"){ num_sh_traj = value; is_num_sh_traj = 1; }
    else if(key=="boltz_flag"){ boltz_flag = value; is_boltz_flag = 1; }
    else if(key=="debug_flag"){ debug_flag = value; is_debug_flag = 1; }
    else if(key=="Temp"){ Temp = value;  is_Temp = 1; }
    else if(key=="nucl_dt"){ nucl_dt = value;  is_nucl_dt = 1; }
    else if(key=="elec_dt"){ elec_dt = value;  is_elec_dt = 1; }
    else if(key=="integrator"){ integrator = value;  is_integrator = 1; }
    else if(key=="runtype"){ runtype = value; is_runtype = 1; }

    else if(key=="alp_bet"){ alp_bet = value; is_alp_bet = 1; }
    else if(key=="decoherence"){ decoherence = value; is_decoherence = 1; }
    else if(key=="regress_mode"){ regress_mode = value; is_regress_mode = 1; }

    else if(key=="is_field"){ is_field = value; is_is_field = 1; }
    else if(key=="field_dir"){ field_dir = value; is_field_dir = 1; }
    else if(key=="field_protocol"){ field_protocol = value; is_field_protocol = 1; }
    else if(key=="field_Tm"){ field_Tm = value; is_field_Tm = 1; }
    else if(key=="field_T"){ field_T = value; is_field_T = 1; }
    else if(key=="field_freq"){ field_freq = value; is_field_freq = 1; }
    else if(key=="field_freq_units"){ field_freq_units = value; is_field_freq_units = 1; }
    else if(key=="field_fluence"){ field_fluence = value; is_field_fluence = 1; }
