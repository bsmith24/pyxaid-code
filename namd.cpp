...
***********************************************/
  ...
  // DaveTurner - Took header info out to save space in files

  timer.Start("deco_rates IO" );

  ofstream out1((rt_dir+"Spectral_density.txt").c_str(),ios::out);
  for(w=0;w<Npoints;w++){
     out1 << w*dE << ' ' << w*dE*8065.54468111324 << ' ' << J[w] << ' ' << sqrt(J[w])      << '\n';
  }
  ...
  // DaveTurner - Commented out dump of Dephasing func since we don't use it
  // Output D and its model(based on the fitted parameters)
