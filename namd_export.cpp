...
int main( int argc, char **argv )
{
  ...
  MPI_Comm_rank( MPI_COMM_WORLD, &mpi::myproc );
  cout << "myproc is " << mpi::myproc << " of " << mpi::nprocs;
...
//>>>>>>>>>>>>>>>>>>>>>>>> INITIALIZATION PART <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  ...
  timer.Start("Reading Hamiltonian");
...
//    }
  ...
  t_namd = clock() - t_namd; 
  cout << "Time in namd is: "<< t_namd / ((double)CLOCKS_PER_SEC) << endl;

  MPI_Finalize( );

  // Report the time profiling information to time.out
